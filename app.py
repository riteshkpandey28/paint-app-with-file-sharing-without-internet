...
def upload_image1():
    if request.method == 'POST':
        ...
        output.close()
        rotate_image = Image.open('./static/uploads/fish1.png')
        final_image = rotate_image.rotate(
            90, expand=1).save('./static/uploads/fish1.png')
...
def upload_image2():
    if request.method == 'POST':
        ...
        output.write(resource.read())
        output.close()
        rotate_image = Image.open('./static/uploads/fish2.png')
        final_image = rotate_image.rotate(
            90, expand=1).save('./static/uploads/fish2.png')
        flash('Image successfully uploaded')
        return redirect(url_for('home'))
